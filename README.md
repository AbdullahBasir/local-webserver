...
2. Run goose up in the files dir to apply it to the database (`goose postgres "connection-string" up`) Sometimes a reset is required to parse a new migration, run (`goose postgres "connection_string" down`) then run (goose up).
...
1. Create a requestBody Struct that holds the request parameters such as the body with JSON tags, 
    `type requestBody Struct {Body string json:"body"}`
